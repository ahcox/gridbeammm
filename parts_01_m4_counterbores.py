...
filename_base = "gridbeam_parts_01_m4_counterbores_"
beam_args = copy.deepcopy(default_beam_args)
beam_args.corner_rounding_outer = 0.25
beam_args.bolt_shaft_diameter = 4.7 # Dial this for innacurate printing..
beam_args.bolt_bore_diameter = 8.0
beam_args.bolt_bore_depth = 2
...
def build_beam(x, y, z):
    result  = build_grid_beam_bored_capped_x(beam_args, x, y, z)
    result = result.clean()
...
result  = build_grid_beam_bored_capped_x(beam_args, 3, 1, 1)
result = result.clean()
filename = filename_base + "03x01x01"
cq.exporters.export(result, filename + ".step")
cq.exporters.export(result, filename +".svg")
